...
class EmbeddingManager:
    ...
    def __init__(self, model_name: Optional[str] = None, base_url: Optional[str] = None):
        ...
        Args:
            ...
            base_url: Ollama server base URL (if using Ollama)
        """
        # Prioritize Local Embeddings (Ollama) as per user request for free plan
        ...
        try:
            ...
            logger.info(f"Initialized Ollama embedding model '{self.model_name}' at {self.base_url}")
            # self._test_embedding() # Optional: Disable if startup speed is a concern
            return
            
        except Exception as e:
            logger.warning(f"Failed to initialize Ollama embeddings: {e}. Trying fallback...")

        # # Fallback to Google Generative AI if API key is available
