...
import org.intervyouai.exception.EmailAlreadyExistsException;
import org.intervyouai.exception.ResourceNotFoundException;
        ...
        if (role == UserRole.ORG_ADMIN) {
            if (signUpRequest.getOrganizationName() == null || signUpRequest.getOrganizationCode() == null) {
                throw new IllegalArgumentException(
                        "Error: Organization name and code are required for ORG_ADMIN registration.");
            }

            if (organizationRepository.existsByCode(signUpRequest.getOrganizationCode())) {
                throw new EmailAlreadyExistsException(
                        "Organization code '" + signUpRequest.getOrganizationCode() + "' is already in use!");
            }
            ...
            organization = organizationRepository.findByCode(signUpRequest.getOrganizationCode())
                    .orElseThrow(() -> new ResourceNotFoundException(
                            "Error: Organization not found with code: " + signUpRequest.getOrganizationCode()));
        }

        User user = User.builder()
                .email(signUpRequest.getEmail())
                .password(encoder.encode(signUpRequest.getPassword()))
                .fullName(signUpRequest.getFullName() != null ? signUpRequest.getFullName()
